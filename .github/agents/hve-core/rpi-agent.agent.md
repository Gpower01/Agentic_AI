...
* Store one `Review decision record` entry with status `current` and evidence containing the review path, latest Parent Decision Record event ID, content revision or hash. `rpi-review` owns reservation, append-only decisions, and recovery semantics; do not duplicate execution, outcome, walkthrough, or route payloads in state.
